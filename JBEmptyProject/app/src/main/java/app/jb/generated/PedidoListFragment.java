...
public class PedidoListFragment extends ListFragment {
	PedidoListFragmentDelegate delegate;
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
    	Pedido obj = listPedido.get(position);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    	if(container != null) {
    		ListView listView = (ListView) container.findViewById(R.id.PedidoListFragmentListView);
    		return listView;
    	}
	...
	public void onAttach(Activity activity) {
		...
		try {
			delegate = (PedidoListFragmentDelegate) activity;
    ...
    public void load() {
		new PedidoListFragment.SendLoadRequestTask().execute();
    }

	private class SendLoadRequestTask extends AsyncTask<String, Integer, String> {
		protected String doInBackground(String... urls) {
    		if(delegate != null) {
    			listPedido = delegate.getListPedido();
    		}
    		else {
		...
		protected void onPostExecute(String result) {
    		textValues = new String[listPedido.size()];
			for(int i = 0; i < listPedido.size(); i++) {
				textValues[i] = DescriptionUtil.extractDescription(listPedido.get(i), DescriptionType.PRIMARY);
				textValues[i] += "___";
				textValues[i] += DescriptionUtil.extractDescription(listPedido.get(i), DescriptionType.SECONDARY);
			}
