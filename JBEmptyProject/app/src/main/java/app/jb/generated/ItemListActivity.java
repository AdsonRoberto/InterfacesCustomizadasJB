...
import app.jb.generated.ItemListFragment;
import org.jb.model.Item;

public class ItemListActivity extends AppCompatActivity implements ItemListFragmentDelegate {

    ItemListFragment list = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		if(type == TypeListView.RESULT) {
    		list_activity_resource = R.layout.activity_item_find_result_list;
    		list_activity_list_fragment_resource = R.id.itemFindResultListActivityFragmentList;
		}
		else if(type == TypeListView.SELECTION) {
    		list_activity_resource = R.layout.activity_item_select_list;
    		list_activity_list_fragment_resource = R.id.itemSelectListActivityFragmentList;
		}
		else  {
    		list_activity_resource = R.layout.activity_item_list;
    		list_activity_list_fragment_resource = R.id.itemListActivityFragmentList;
		}
    	...
    	if (fm.findFragmentById(list_activity_list_fragment_resource) == null) { 
    		...
    		list = (ItemListFragment) fm.findFragmentById(list_activity_list_fragment_resource);
    		list.onAttach(this); 
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
    	...
    	TypeListView type = (TypeListView) getIntent().getSerializableExtra("TYPE_LIST_VIEW");
		if(type == TypeListView.RESULT) {
    		getMenuInflater().inflate(R.menu.context_menu_find_result_list_item_activity, menu);
		}
		else if(type == TypeListView.SELECTION) {
    		getMenuInflater().inflate(R.menu.context_menu_select_list_item_activity, menu);
		}
		else {
    		getMenuInflater().inflate(R.menu.context_menu_list_item_activity, menu);
		}
    }
    ...
    public boolean onContextItemSelected(MenuItem item) {
    	...
    	if (id == R.id.context_menu_list_item_activity_action_detail) {
    		openDetailSelectedItem(index);
    		return true;
    	}
    	if (id == R.id.context_menu_list_item_activity_action_edit) {
    		openEditSelectedItem(index);
    		return true;
    	}
    	if (id == R.id.context_menu_list_item_activity_action_delete) {
    		openDeleteSelectedItem(index);
    		return true;
    	}
    	if (id == R.id.context_menu_select_list_item_activity_action_select) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
    	...
    	if (id == R.id.menu_list_item_activity_action_new) {
    		Intent intent = new Intent(this, ItemInsertActivity.class);
    		startActivity(intent);
    		return true;
    	}
    	if (id == R.id.menu_list_item_activity_action_find) {
    		Intent intent = new Intent(this, ItemFindActivity.class);
    		startActivity(intent);
    		return true;
    	}
    ...
    public void openDeleteSelectedItem(int index) {
    	...
    	alert.setTitle(R.string.activity_title_item_list);
	...
	public List<Item> getListItem() {
		...
		try {
			if(type == TypeListView.RESULT) {
				...
				listItem = (List<Item>) dao.findAll(Item.class);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private class SendRemoveRequestTask extends AsyncTask<String, Integer, String> {
		protected String doInBackground(String... urls) {
			int indexItem = getIntent().getIntExtra("indexItem", 0);
			try {
				WebDAO dao = new WebDAO();
				return dao.delete(Item.class, indexItem);
